...
export default function LessonEditorBeta({ onBack, onSelectLesson, initialLessonId = '', viewMode = 'all' }) {
  ...
  const sentenceList = useMemo(() => segmentLessonText(sourceText), [sourceText]);
  ...
  const loadLesson = (lessonId) => {
    ...
    setLessonOrder(lesson.order ? String(lesson.order) : '');
  ...
  const runOcrFromFiles = async (files) => {
    ...
    try {
      ...
      setSourceText(normalizeLessonText(normalizeSourceByLine(cleanRaw), 'structured'));
  ...
  const saveLesson = () => {
    ...
    const normalizedSource = normalizeLessonText(sourceText, structuredMode ? 'structured' : 'free');
    ...
    const payload = {
      title: title.trim(),
      order: normalizeLessonOrder(lessonOrder, undefined),
  ...
  const debugSourceText = () => {
    ...
    const segments = segmentLessonText(raw);
    console.log('Raw:', raw);
    console.log('JSON:', json);
    console.log('Parsed:', parsed);
    console.log('Segments:', segments);
    window.alert(
      `Caracteres: ${raw.length}\nRetours ligne: ${(raw.match(/\n/g) || []).length}\nPhrases: ${segments.length}`,
    );
  };
        ...
        <section className="lesson-beta-card lesson-pro-col source-col">
            ...
            <label className="lesson-field">
              ...
              <select value={selectedLessonId} onChange={(e) => loadLesson(e.target.value)}>
                ...
                {orderedLessons.map((lesson) => (
                  <option key={lesson.id} value={lesson.id}>{lesson.order ? `${lesson.order}. ` : ''}{lesson.title}</option>
                ))}
              </select>
            </label>
            <label className="lesson-field">
          ...
          <div className="lesson-beta-actions">
            <label className="lesson-field checkbox-field lesson-field-inline">
              <span>Mode structure (1 phrase / ligne)</span>
              <input
                type="checkbox"
            ...
            <button type="button" className="button secondary ui-pressable" onClick={debugSourceText}>
              🔍 Debug SourceText
            </button>
          </div>
          ...
          <label className="lesson-field">
            ...
            <textarea
              ...
              placeholder={structuredMode ? '老师您早！\n您早！\n老师您好！\n您好！' : '老师您早！您早！老师您好！您好！'}
              style={{
                whiteSpace: 'pre-wrap',
                lineHeight: structuredMode ? 2 : 1.5,
              }}
            />
          </label>
          ...
          <div className="segment-preview">
            <h4>📋 Phrases detectees ({sentenceList.length})</h4>
            {sentenceList.length ? (
              sentenceList.map((sentence, index) => (
                <div key={`${sentence}-${index}`} className="sentence-item">
                  {index + 1}. {sentence}
                </div>
              ))
            ) : (
              <div className="sentence-item muted">Aucune phrase detectee.</div>
            )}
          </div>
