...
function expandLessonFromV2(lesson) {
  ...
  // If already in expanded format (cards present, no characterRefs), normalize directly
...
function buildCharacterRefsFromLesson(cards = [], sourceText = '') {
  const ordered = buildCharacterRefsFromCards(cards);
  extractUniqueChars(sourceText).forEach((char) => {
    if (!ordered.includes(char)) ordered.push(char);
  });
  return ordered;
}
...
function upsertCharacterFromSource(char, lessonId) {
  if (!char) return;
  const existing = getCharacter(char);
  const appears = new Set(existing?.appearsInLessons || []);
  appears.add(lessonId);
...
export function LessonsProvider({ children }) {
  ...
  const createLesson = ({
    ...
    const characterRefs = buildCharacterRefsFromLesson(cards, sourceText);
    const lesson = createLessonV2({
      title,
      order: nextOrder,
      description,
    ...
    extractUniqueChars(sourceText).forEach((char) => upsertCharacterFromSource(char, lesson.id));
  ...
  const updateLesson = (lessonId, patch = {}) => {
    ...
    const characterRefs = cards
      ? buildCharacterRefsFromLesson(cards, patch?.sourceText || '')
      : patch?.sourceText
        ? buildCharacterRefsFromLesson([], patch.sourceText)
        : undefined;
    const updates = {
      ...patch,
    ...
    if (patch?.sourceText) {
      extractUniqueChars(patch.sourceText).forEach((char) => upsertCharacterFromSource(char, lessonId));
    }
    const refreshed = getAllLessonsV2().map((lesson) => expandLessonFromV2(lesson));
    setLessons(getLessonsByOrder(refreshed));
  };
